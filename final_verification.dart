...
void main() async {
  ...
  final defs = (data['banks'] as List)
      .map((json) => BankDefinition.fromJson(json))
      .toList();
  ...
  for (var bankPrefix in banksToTest) {
    final samples = sampleSms
        .where((s) => s.sender.toUpperCase().contains(bankPrefix))
        .toList();
    ...
    for (var s in samples) {
      ...
      if (tx != null) {
        print(
          'PARSED: ${tx.type} | Amount: ${tx.amount} | Balance: ${tx.availableBalance} | Template: ${tx.templateName}',
        );
        parsed++;
        totalParsed++;
      } else {
