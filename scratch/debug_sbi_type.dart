...
void main() async {
  ...
  print('--- SBI CREDIT/DEBIT VERIFICATION ---');
  int total = 0;
  int success = 0;

  for (var sms in sampleSms) {
    if (sbiDef.canHandle(sms.sender)) {
      total++;
      final tx = parser.parse(sms.body);
      if (tx != null) {
        success++;
        final typeStr = tx.type.toString().split('.').last.toUpperCase();
        print('SUCCESS: [$typeStr] [${sms.sender}] ${sms.body}');
      } else {
