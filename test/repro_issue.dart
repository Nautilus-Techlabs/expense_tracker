...
import 'package:expense_tracker/domain/parsers/parser_config_service.dart';
...
void main() async {
  test('Verification for Federal and Dhanlaxmi Banks', () async {
    ...
    final configString = await configFile.readAsStringSync();
    ...
    final fedSamples = sampleSms.where((s) => s.sender.contains('FEDBK')).toList();
    for (var sample in fedSamples) {
      final tx = engine.tryParse(sample.body, sender: sample.sender);
      print('\nSMS: ${sample.body}');
      if (tx != null) {
        print('✅ Parsed -> Amount: ${tx.amount}, Balance: ${tx.availableBalance}, Bank: ${tx.bankName}');
      } else {
    ...
    final dlxSamples = sampleSms.where((s) => s.sender.contains('DLXBNK')).toList();
