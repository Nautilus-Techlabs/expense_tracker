...
void main() {
  ...
  final p1 = RegExp(r"(?:INR|Rs\.?)\s*([0-9,]+(?:\.\d+)?)\s+debited\s+from\s+", caseSensitive: false);
  print('Part 1 match: ${p1.hasMatch(sms)}');
  
  // Part 2: Up to Account
  final p2 = RegExp(r"(?:INR|Rs\.?)\s*([0-9,]+(?:\.\d+)?)\s+debited\s+from\s+.*?A/c\s+(?:no\.\s+)?(?:ending\s+)?([\dX*]+)", caseSensitive: false);
  ...
  // Part 3: Full
  final p3 = RegExp(r"(?:INR|Rs\.?)\s*([0-9,]+(?:\.\d+)?)\s+debited\s+from\s+.*?A/c\s+(?:no\.\s+)?(?:ending\s+)?([\dX*]+).*?UPI Ref:", caseSensitive: false);
